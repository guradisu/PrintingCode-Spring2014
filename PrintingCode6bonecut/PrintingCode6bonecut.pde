...
void setup()
{
  ...
  for (int i=0;i<13;i++){
    ...
    if (i==0){
      ...
      angle[i] = angle[i-1]+random(angleBase[i]*0.8, angleBase[i]*1.5);
    }
  ...
  background(240,160,131);
  colorMode(HSB,1,1,1);
  
  RG.init(this);
  RFont futura = new RFont("Futura-Medium.ttf", 50, RFont.CENTER);
 ...
 // meat surface
  ...
  // bone in
  pushMatrix();
  translate(rind*(radius[12]*cos(angle[0])+radius[1]*cos(angle[1]))/2, -20);
  fill(0,0.04,0.98);
  float x,y;
  beginShape();
    for (int i=0;i<7;i++){
      x = 25*cos(2*PI/7*i+random(-0.3,0.3));
      y = 20*sin(2*PI/7*i+random(-0.3,0.3));
      vertex(x,y);
    }
  endShape(CLOSE);
  popMatrix();
